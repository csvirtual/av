...
// entre a tela de bloqueio por trial expirado (app.js#renderLicenseBlockedScreen)
// e o atalho "Solicitar chave" em Dados da loja (views/company.js), que
// precisava do mesmo fluxo mas ANTES do trial/demo ter expirado (ver
// openSupportContactChoiceModal mais abaixo) — por isso a mensagem recebe
// `reasonText` como parâmetro em vez de vir fixa com "meu período de teste
// encerrou", frase que seria falsa vinda de Dados da loja com o trial/demo
// ainda válido.
...
// pra ir direto pedir a chave ao suporte — mesmo fluxo que já existia (só)
// na tela de bloqueio por trial expirado, mas alcançável a qualquer momento
// (mesmo com trial/demo ainda válido), sem precisar esperar travar. Este
// modal escolhe WhatsApp ou e-mail; cada opção abre o modal já existente
// (openSupportWhatsappModal/openSupportEmailModal, com a mensagem/chips de
// sempre), fechando este primeiro — mesmo padrão em dois passos que a tela
// de bloqueio já usa (ícones -> modal com a mensagem pronta), só que aqui os
// dois ícones moram dentro de um modal em vez de soltos na tela cheia (a
// caixa de Ativação em Dados da loja é pequena demais pra caber os dois
// ícones direto, sem competir com o resto do card).
