...
const FAQ_CATEGORIES = [
  {
    ...
    items: [
      ...
      },
      {
        ...
        a: 'Esse é o mais confuso de entender na hora, porque não parece ter relação com o que você está fazendo: o carrinho de uma venda <strong>não se limpa sozinho ao trocar de tela</strong>, só ao finalizar a venda ou ao deslogar (veja "O carrinho não some se você trocar de tela" no tópico Vendas). Se, com itens já no carrinho, alguém for no Estoque e <strong>excluir</strong> um desses produtos, ao voltar pra "Nova venda" e tentar finalizar, o sistema corretamente barra a venda, nomeando o produto certo. Nos dois casos, a solução é a mesma: clique no <strong>×</strong> pra remover aquele item específico do carrinho e adicione o produto de novo pela busca; a venda fecha normal depois disso.<div class="tip"><strong>Dica:</strong> desde a v1.15.26, tentar <strong>excluir</strong> um produto no Estoque enquanto ele está parado num carrinho aberto já mostra um aviso na hora, antes de excluir de verdade, evitando cair nessa situação.</div>',
      },
      {
...
const TOPICS = [
  {
    ...
    html: `
      ...
      <p>Em <strong>Dados da loja</strong> tem também a seção <strong>Ramo de atuação</strong>: checkboxes de "Material de construção" e "Mercearia", só informativo (não muda nenhum cálculo nem trava nenhuma tela), marque quantos combinarem com o seu negócio.</p>
  ...
  },
  {
    ...
    html: `
      ...
      <div class="tip"><strong>Migrando do pdv-servidor (a versão multi-terminal) pra esta extensão?</strong> As duas usam exatamente o mesmo formato de colunas: um CSV exportado de um serve de entrada pro outro sem nenhuma adaptação. É o caminho recomendado pra trazer o catálogo de um produto pro outro, já que o backup de cada um (veja o tópico "Backup") não é compatível entre eles. Única diferença: esta extensão só tem as categorias "material" e "mercearia"; uma linha vinda do servidor com a categoria "loja" (que só existe lá) entra aqui como "material" automaticamente, sem dar erro.</div>
