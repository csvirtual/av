...
import { escapeHtml, onlyDigits, UFS } from '../utils/format.js';
import { MIN_USER_PASSWORD_LENGTH } from '../utils/permissions.js';
...
const THEME_OPTIONS = [
  { value: 'system', icon: icon('desktop', { size: 20 }), label: 'Automático', desc: 'Segue o tema do sistema operacional' },
  { value: 'light', icon: icon('sun', { size: 20 }), label: 'Claro', desc: 'Sempre com fundo claro' },
  { value: 'dark', icon: icon('moon', { size: 20 }), label: 'Escuro', desc: 'Sempre com fundo escuro' },
];
    ...
    shell(`
      ...
      <div class="setup-choice-grid">
        <button type="button" class="setup-choice-card" id="choice-new">
          <span class="icon">${icon('store', { size: 22 })}</span>
        ...
        <button type="button" class="setup-choice-card" id="choice-restore">
          <span class="icon">${icon('save', { size: 22 })}</span>
  ...
  function renderRestore() {
    shell(`
      <form id="restore-form" novalidate>
        ...
        <div class="modal-actions" style="justify-content:space-between;padding:8px 0 0;">
          <button type="button" class="btn btn-secondary" id="back-btn">${icon('arrowLeft', { size: 14 })} Voltar</button>
    ...
    shell(`
      <form id="company-form" novalidate>
        ...
        <div class="form-row">
          <div class="field">
            <div class="field-label-row">
              ...
              ${infoTooltipHtml('cnpj-legal-notice-setup', 'Mais informações sobre o CNPJ', CNPJ_LEGAL_NOTICE_HTML)}
            </div>
            <input id="cnpj" name="cnpj" placeholder="xx.xxx.xxx/xxxx-xx" maxlength="18" required>
            <span class="hint" id="cnpj-hint"></span>
          </div>
          <div class="field">
            <label for="telefone">Telefone *</label>
            <input id="telefone" name="telefone" placeholder="(xx) x xxxx-xxxx" maxlength="17" required>
          </div>
        </div>
        <div class="form-row">
          <div class="field">
        ...
        <div class="form-row">
          ...
          <div class="field" style="flex:0 0 130px"><label for="cep">CEP *</label><input id="cep" placeholder="xxxxx-xxx" maxlength="9" required></div>
        </div>
        ...
        <div class="modal-actions" style="justify-content:space-between;padding:8px 0 0;">
          <button type="button" class="btn btn-secondary" id="back-btn">${icon('arrowLeft', { size: 14 })} Voltar</button>
          <button type="submit" class="btn">Continuar ${icon('arrowRight', { size: 14 })}</button>
        </div>
      </form>
    `, {
      title: 'Dados da loja',
  ...
  function renderStep2() {
      ...
      <form id="admin-form" novalidate>
        ...
        <div class="form-row">
          <div class="field">
            ...
            <input id="password" type="password" required minlength="${MIN_USER_PASSWORD_LENGTH}">
          </div>
          <div class="field">
            <label for="confirmPassword">Confirmar senha *</label>
            <input id="confirmPassword" type="password" required minlength="${MIN_USER_PASSWORD_LENGTH}">
          </div>
        </div>
        <div class="modal-actions" style="justify-content:space-between;padding:8px 0 0;">
          <button type="button" class="btn btn-secondary" id="back-btn">${icon('arrowLeft', { size: 14 })} Voltar</button>
    ...
    form.addEventListener('submit', async (e) => {
      ...
      if (password.length < MIN_USER_PASSWORD_LENGTH) {
        errBox.innerHTML = `<div class="form-error">A senha deve ter pelo menos ${MIN_USER_PASSWORD_LENGTH} caracteres.</div>`;
        return;
      }
