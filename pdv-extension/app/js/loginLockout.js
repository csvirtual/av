...
function keyFor(username) {
  return `loginLockout:${(username || '').trim().toLowerCase()}`;
}
...
export async function getLoginLockState(username) {
...
export async function clearLoginLock(username) {
  if (!username) return;
  await chrome.storage.local.remove(keyFor(username));
}
