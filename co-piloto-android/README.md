...
- **`build.js`** — monta tudo isso num HTML só, lendo os arquivos reais de
  `../co-piloto` (nunca uma cópia à mão).
...
- **PWA de verdade**: falta o `manifest.webmanifest` + service worker
  mínimo pra instalabilidade ("Adicionar à tela inicial" no Android) —
  ainda é só um HTML que abre no navegador, não um app instalável.
  `build.js` ainda não gera isso.
- **Hospedagem**: precisa subir `dist/copiloto-android.html` em algum
  lugar com HTTPS (decisão já tomada com o usuário — não um arquivo local
  aberto direto do disco).
