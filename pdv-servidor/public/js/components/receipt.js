...
function buildReceiptHtml(sale, company, customerName) {
  ...
  const itemsHtml = sale.items.map((i) => {
    const refunded = i.qtyRefunded > 0 ? ` <span class="r-item-refunded">(${formatQty(i.qtyRefunded)} estornado${i.qtyRefunded > 1 ? 's' : ''})</span>` : '';
    ...
    return `
      <div class="r-item-row">
        ...
        <div class="r-cell r-cell-qty">${formatQty(i.qty)} ${escapeHtml(i.unit)}</div>
  ...
  return `
    <div class="receipt">
      <div class="r-header">
        ...
        ${company?.telefone ? `<div class="r-small">Tel: ${escapeHtml(formatPhoneBR(company.telefone))}</div>` : ''}
      </div>
...
export function printSaleReceipt(sale, company, customerName) {
  printFromRoot('print-receipt-root', buildReceiptHtml(sale, company, customerName));
}
