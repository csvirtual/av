...
import { printFromRoot } from './printRoot.js';
...
function buildReportPrintHtml({ report, periodLabel, company }) {
  ...
  return `
    <div class="rp-sheet">
      <div class="rp-header">
        <div>
          <div class="rp-store-name">${escapeHtml(company?.nomeFantasia || 'PDV - C&S Virtual')}</div>
...
export function printReport({ report, periodLabel, company }) {
  printFromRoot('print-report-root', buildReportPrintHtml({ report, periodLabel, company }));
}
