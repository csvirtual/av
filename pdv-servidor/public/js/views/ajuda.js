...
const FAQ_CATEGORIES = [
  {
    ...
    items: [
      ...
      },
      {
        ...
        a: 'Esse é o mais confuso de entender na hora, porque não parece ter relação com o que você está fazendo: o carrinho de uma venda <strong>não se limpa sozinho ao trocar de tela</strong>, só ao finalizar a venda ou ao deslogar (veja "O carrinho não some se você trocar de tela" no tópico Vendas). Se, com itens já no carrinho, alguém for no Estoque e <strong>excluir</strong> um desses produtos, ao voltar pra "Nova venda" e tentar finalizar, o sistema corretamente barra a venda, nomeando o produto certo. Nos dois casos, a solução é a mesma: clique no <strong>×</strong> pra remover aquele item específico do carrinho e adicione o produto de novo pela busca; a venda fecha normal depois disso.<div class="tip"><strong>Dica:</strong> tentar <strong>excluir</strong> um produto no Estoque enquanto ele está parado num carrinho aberto já mostra um aviso na hora, antes de excluir de verdade, evitando cair nessa situação.</div>',
      },
      {
    ...
    items: [
      {
        ...
        a: 'Um cliente com saldo devedor em aberto não pode ser excluído, pois apagaria o rastro de uma dívida real. Registre o pagamento do fiado (zerando o saldo) antes de excluir, ou use <strong>Inativar</strong> no lugar (esconde da lista de venda sem apagar o histórico nem a dívida).',
      },
      {
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        a: 'Assim que o primeiro item de um pedido de compra é marcado como recebido (e o estoque já sobe com ele), o pedido inteiro deixa de poder ser cancelado, pois cancelar apagaria o rastro de uma entrada de estoque real já aplicada. Pedidos ainda 100% pendentes (nada recebido) continuam podendo ser cancelados normalmente.',
      },
    ],
  },
  {
...
const TOPICS = [
  {
    ...
    html: `
      ...
      <div class="tip"><strong>Errou a senha 2 vezes seguidas?</strong> O campo de senha fica bloqueado por 60 segundos (com contagem regressiva na tela) antes de liberar de novo, como proteção contra tentativa de adivinhar a senha de outra pessoa no teclado. Não afeta o usuário digitado nem apaga nada; é só esperar a contagem zerar.</div>
      ...
      <div class="tip"><strong>Ficou parado 30 minutos?</strong> O sistema desloga sozinho depois de meia hora sem nenhum uso (mouse, teclado etc.), voltando pro login, como proteção pra quem sai do balcão e esquece o sistema logado. Se tiver mais de uma aba aberta, mexer em qualquer uma delas conta como uso; só desloga de verdade quando nenhuma aba tiver atividade recente.</div>
  ...
  },
  {
    ...
    html: `
      ...
      <h3>Exportar e importar o catálogo em CSV</h3>
      <p>O botão <strong>CSV ▾</strong>, no topo da tela de Estoque, abre um menu com duas opções. <strong>Exportar CSV</strong> baixa uma planilha com o catálogo inteiro (ativos e inativos, ignorando qualquer filtro ativo na tela): nome, categoria, unidade, código de barras, preço, custo, estoque, estoque mínimo, fornecedor, ativo/inativo, validade e promoção. <strong>Importar CSV</strong> (só aparece pra quem tem "Cadastrar/editar produto") lê essa mesma planilha de volta: cada linha com um <strong>código de barras novo</strong> vira um produto cadastrado do zero, já com a quantidade da planilha como estoque inicial; cada linha com um código de barras <strong>que já existe</strong> só atualiza os dados de cadastro (nome, preço, categoria...). A quantidade em estoque de um produto que já existe <strong>nunca</strong> é alterada pela importação, de propósito, pra reimportar a mesma planilha (ex: só pra corrigir um preço) nunca arriscar duplicar nem resetar o estoque de verdade. No final aparece um resumo: quantos produtos foram criados, quantos atualizados, e uma lista de qualquer linha com problema (código de barras faltando, fornecedor não encontrado etc.). O que deu certo é salvo mesmo se outra linha do mesmo arquivo tiver erro.</p>
      <div class="tip"><strong>Migrando da extensão Chrome pra este servidor (ou vice-versa)?</strong> As duas usam exatamente o mesmo formato de colunas: um CSV exportado de um serve de entrada pro outro sem nenhuma adaptação. É o caminho recomendado pra trazer o catálogo de um produto pro outro, já que o backup de cada um (veja o tópico "Backup") não é compatível entre eles.</div>
  ...
  },
  {
    ...
    html: `
      ...
      <div class="warn-box"><strong>Mas recarregar a página (F5) apaga o carrinho, inclusive os congelados.</strong> Diferente de trocar de tela (que é só navegar dentro do sistema, sem sair dele), apertar F5 ou fechar e reabrir a aba faz o navegador recarregar tudo do zero: o carrinho, o desconto e os pagamentos já adicionados existem só na memória daquele momento, em nenhum lugar salvo (nem no computador, nem no servidor), então se perdem junto. A sessão de login continua a mesma (não desloga), só o que estava no meio da venda é que some. Evite recarregar a página com uma venda em andamento; se acontecer sem querer, será preciso montar o carrinho de novo.</div>
      ...
      <div class="tip"><strong>Estoque reservado: só neste terminal.</strong> Um produto com pouca quantidade sobrando não pode ser vendido duas vezes NESTE MESMO computador: se 3 das 5 unidades já estão num carrinho congelado aqui, o próprio terminal só deixa pegar as 2 que sobraram. Mas essa "reserva" não vale pros outros terminais: se outro vendedor, em outro computador, vender as 5 unidades antes de você voltar nesse carrinho congelado, a venda dele passa normal (o sistema nunca deixa faltar estoque de verdade, então quem chegar primeiro leva), e é a SUA venda que vai falhar na hora de finalizar, com um aviso de estoque insuficiente. Aí você descobre, na frente do cliente, que o produto já não está mais disponível. Pra produto com pouco estoque e mais de um terminal ativo, combine com a equipe antes de congelar um carrinho grande por muito tempo.</div>
      <div class="tip"><strong>Não fica salvo se fechar o navegador, nem se recarregar a página (F5).</strong> Carrinho congelado vive só na memória do navegador, igual o carrinho normal (veja o aviso logo acima, em "Adicionar produtos ao carrinho"): some ao deslogar, trocar de usuário, fechar a aba, OU simplesmente apertar F5. Não é lugar pra guardar uma venda de um dia pro outro, nem pra deixar parado por muito tempo sem cuidado com a própria aba.</div>
...
${linhaEncarregado}</div>
  ...
  },
  {
    ...
    html: `
      ...
      <p>Na tela <strong>Nova venda</strong>, o botão <strong>Finalizar venda + carreto</strong> (abaixo do "Finalizar venda" normal) registra a venda e já cadastra o carreto na sequência, com os itens que você marcar numa lista, poupando ter que abrir a tela Carreto separadamente logo depois de vender. Um carreto cadastrado assim mostra "gerado junto com uma venda" no detalhe. Veja o tópico "Vendas (PDV)" pra mais detalhes desse atalho.</p>
    `,
  },
  {
    id: 'estorno-troca',
  ...
  },
  {
    ...
    html: `
      ...
      <p>O administrador pode ligar, em <strong>Dados da loja</strong>, a opção "Exigir caixa aberto para registrar vendas". Ligada essa opção, ninguém consegue finalizar uma venda sem abrir o caixa primeiro.</p>
