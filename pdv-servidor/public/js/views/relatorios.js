// Relatórios gerenciais — exige a permissão 'relatorios' (admin sempre tem;
// um vendedor só se marcado no cadastro dele, ver utils/permissions.js).
// Envolve margem de lucro (calculada a partir do preço de custo, um dado
// sensível).
import { computeSalesReport } from '../data/reportsRepo.js';
import { getCompany } from '../data/companyRepo.js';
import { formatMoney, formatDate, escapeHtml, formatQty, categoryLabel } from '../utils/format.js';
...
export async function renderRelatorios(container) {
  container.innerHTML = `
    ...
    <div class="toolbar">
      ...
      <button class="btn btn-secondary btn-sm" id="export-report-pdf-btn" type="button">${icon('printer', { size: 15 })} Exportar PDF</button>
        ...
        ${r.byCategory.length === 0 ? '<div class="table-empty">Nenhuma venda no período.</div>' : `
          <div class="table-wrap">
            <table>
              ...
              <tbody>
                ${r.byCategory.map((c) => `<tr><td>${categoryLabel(c.category)}</td><td>${formatQty(c.qty)}</td><td>${formatMoney(c.revenue)}</td></tr>`).join('')}
              </tbody>
            </table>
          </div>
        `}
